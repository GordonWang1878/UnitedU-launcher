...
| 7 | 行改名 | 复用「修改标题」对话框(`TitleDialog.kt`,系统输入法),改成与卡片无关的通用签名;确定保存,**清空 = 不改**(行名不能为空),长度上限沿用 `MAX_TITLE_CHARS` = 40;**改名不改图标**:没存过图标的旧行改名时,先把当前按名字回落显示的图标 id 存下来(实施中发现 MUSIC 改名后会翻成电视图标,2026-09-19 Ruling M4b-R10) | DESIGN §2(自由改,系统输入法)+ Ruling G |
| 8 | 行上下移 | 与相邻行交换;焦点跟着这一行走(落在它的「+」) | Ruling A |
| 9 | 原地移动 | 长按菜单「移动位置」→ 首页进入移动态:被移动的卡保持聚焦,屏幕底部一行提示「← → 移动 · ↑ ↓ 换行 · 确定 放下 · 返回 取消」;左右与同行邻卡换位(到头不动),上下移到相邻的**首页可见应用行**的同一列(越过行尾则放到行尾;跳过输入源行);确定 → 写盘并退出;返回 → 原样放回并退出;按 HOME、进待机、任何浮层要打开 → 等同取消。移空的行在首页随即消失(与今天空行不显示一致),取消时复原。旧的「跳编辑页定位」兜底删除。实施中补定(2026-09-19,Rulings M4b-R14–R16):确定键**松开**才放下,长按确定无反应;移动中音量键照常;放下只重写可见顺序变了的行,没动过的行原样保留;往已有同一应用的行里移 = 原地不动 | DESIGN §2 + Ruling H(细节) |
...
| 18 | 编辑页搬卡(真机验收后补定) | 编辑页卡片菜单的「左移 / 右移」换成一项「移动位置」,进入与首页同一套搬运:← → 与同行邻卡换位,↑ ↓ 换到相邻行的同一列(越过行尾放行尾;**空行也是落点**,源行被移空照样保留),相邻行已有同一应用 = 不动;确定键松开放下(写盘),长按无反应,返回取消(复原不写盘);离开编辑页 / 退到后台 = 取消。起因:新建的空行在首页不显示,首页搬运进不去,只能「移出 + 添加」 | Gordon 2026-09-20 选「编辑页也用搬运模式」 |
| 19 | 多个调谐器合并(真机验收后补定) | 输入源里的调谐器(非透传输入)不止一个时只显示一张:id 不含 analog 的优先,同类按 id 取第一个(A95L 上留索尼 DVB 数字调谐器 HW0,去掉联发科模拟调谐器 HW1)。点调谐器卡本来就只打开系统通用频道列表、不分是哪一个,两张「电视」效果完全一样 | Gordon 2026-09-20 选「合并成一张「电视」」 |
