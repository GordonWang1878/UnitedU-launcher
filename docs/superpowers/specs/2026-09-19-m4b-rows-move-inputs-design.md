...
| 13 | 输入源数据 | 新文件 `inputs.json`(与 `titles.json` 同目录,`Paths.base`):`{"hidden":["<输入 id>",…],"names":{"<输入 id>":"名字",…}}`。与 `titles.json` 同一套读写姿势(坏文件改名留证、写 tmp + fsync + rename)。**「恢复默认」不碰它**(与 titles.json 一致) | Ruling L |
...
- **L 独立 `inputs.json`**:改名 / 隐藏属于用户数据,和卡片标题一样不该被「恢复默认」抹掉。
...
`{"hidden":["id1"],"names":{"id2":"游戏机"}}`,`Paths.inputsJson(ctx) = File(base, "inputs.json")`。`InputPrefs(hidden: Set<String>, names: Map<String, String>)` + 纯函数 `applyInputPrefs(entries, prefs)`(先去重、再去隐藏、再换名)。读写照 `Titles.kt` 的姿势。
...
- 状态:`MainActivity` 持有 `moving: MoveState?`——`MoveState(pkg, rows: List<Row>, original: List<Row>)`,`rows` 是**首页已渲染的行**的工作副本(不重读磁盘、不重解码图标,每一步只是列表重排)。
...
- 纯函数(JVM 单测):`moveLeft/moveRight/moveUp/moveDown(rows, pkg): List<Row>`,规则见 §0-9;输入源行(`RowKind.INPUTS`)永远不是目标行。
...
- 首页输入源行:`Inputs.load` → `dedupeCec` → `applyInputPrefs`(隐藏的去掉、改过名的换名)。全部隐藏 → 这一行不渲染(与今天「没有输入」同一条路)。
- 长按输入源卡 → 菜单(打开 / 改名 / 隐藏),`CardMenu.kt` 的 `cardMenuActions(RowKind.INPUTS)` 返回这三项的新枚举值。菜单打开时焦点离开卡片,按压态随失焦释放(WORKLOG M8 遗留的现象由此消失;真机验)。
...
5. 单测:行操作纯函数、`inputs.json` 解析、`dedupeCec`、`applyInputPrefs`、移动纯函数。
