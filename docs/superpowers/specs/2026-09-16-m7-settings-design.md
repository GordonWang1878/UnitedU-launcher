...
状态:已实施并本地合并进 main(合并提交 `c551568`,分支 `m7-settings` 已删);终审修复波已完成(见 WORKLOG「终审修复波」);A95L 真机第一轮 5 项通过(2026-09-17),HOME 拉起与检查更新相关项待切默认桌面、首个 release 后再测。基线 = main + `theme-cleanup` 分支(删主题化壁纸、主题色全面接线)合并之后。
...
## 10.5 不变量:主题色不进卡片中间(2026-09-16 Gordon 真机确认)

补做的 banner(纯图标卡的回落底 = 图标**边缘色** `edgeColor`)填充色在加载时按图标像素算定、存进 `AppEntry.fallbackColor`,**普通主题色永不改它**(金/蓝预设下实测同为该图标边缘色,纹丝不动)。主题 `accent` 在卡片上只作用两处:① 聚焦卡的边缘呼吸光影;②「主题化卡片」开启时整卡去色染色。**绝不允许**普通主题色染到卡片中间的填充。重构设置页 / 卡片渲染时必须守住这条。
