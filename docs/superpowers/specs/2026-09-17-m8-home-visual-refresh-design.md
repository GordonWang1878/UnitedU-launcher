...
与 M7 的关系(2026-09-18 终审改定,取代本行 2026-09-17 原稿「列为 M8 收尾任务」的说法):**M7 已并入 main**;M8 只改**首页与共用 token**;设置页、编辑页、选择器、菜单、对话框的换皮**不是 M8 的收尾任务**,另立后续 plan(**M8b**)。
...
| 纵向导航 | **焦点行锚定在下三分之一,上面的行推入 hero 区,scrim 随之上移**(当时称为「Google TV 首页做法」——**2026-09-19 更正:未经核实,模拟器上的 Android TV Home 7.7.15 实测是焦点行贴屏幕顶部**,见 `docs/research/2026-09-19-google-tv-home-gap.md`;是否改为贴顶待 Gordon 定) | 3 行放不进下三分之一;固定窗口一屏只见一行 |
